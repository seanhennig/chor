...
// { variables
// structure
// removed zUhrikova 03/02/2010
// slash from the first '':
...
//  change zUhrikova feb 1, 2010 Foliovision
$kfm->defaultSetting('hidden_panels',array('logs','directory_properties'));
...
// depricated
$kfm->defaultSetting('allow_image_manipulation',0); // this is plugin management, changed to 0 zUhrikova
...
if(!substr($kfm_userfiles_output_real,-1,1)=='/' && strlen( $kfm_userfiles_output_real ) > 0 )$kfm_userfiles_output_real .= '/'; // Just convention, end with slash
if(!substr($kfm_userfiles_address,-1,1)=='/')$kfm_userfiles_address .= '/'; // Just convention, end with slash
$kfm->defaultSetting('files_url',$kfm_userfiles_output);
$kfm->defaultSetting('real_files_url',$kfm_userfiles_output_real );
$kfm->defaultSetting('filter_wp_thumbnails', $aFckOptions["filter_wp_thumbnails"]);
// { KFM versions
...
// }
$rootdir = (strpos($kfm_userfiles_address, './')===0 || strpos($kfm_userfiles_address, '../')===0) ?KFM_BASE_PATH.$kfm_userfiles_address:$kfm_userfiles_address.''; //zUhrikova last slash removed

if (!is_dir($rootdir))mkdir($rootdir, octdec( '0' . $kfm->setting('default_directory_permission') ), true);
...
if (is_dir($workpath)) {
    ...
    if (is_writable(dirname($workpath_tmp)))mkdir($workpath_tmp, octdec( '0' . $kfm->setting('default_directory_permission') ) );
    else{
...
if($image_path_changed){
   $kfm->db->query('DROP TABLE '.$kfm_db_prefix.'directories');
   $kfm->db->query('DROP TABLE '.$kfm_db_prefix.'files');
   $kfm->db->query('DROP TABLE '.$kfm_db_prefix.'files_images');
   $kfm->db->query('DROP TABLE '.$kfm_db_prefix.'files_images_thumbs');
   $kfm->db->query('DROP TABLE '.$kfm_db_prefix.'parameters');
   $kfm->db->query('DROP TABLE '.$kfm_db_prefix.'plugin_extensions');
   $kfm->db->query('DROP TABLE '.$kfm_db_prefix.'session');
   $kfm->db->query('DROP TABLE '.$kfm_db_prefix.'session_vars');
   $kfm->db->query('DROP TABLE '.$kfm_db_prefix.'settings');
   $kfm->db->query('DROP TABLE '.$kfm_db_prefix.'tagged_files');
   $kfm->db->query('DROP TABLE '.$kfm_db_prefix.'tags');
   $kfm->db->query('DROP TABLE '.$kfm_db_prefix.'translations');
   $kfm->db->query('DROP TABLE '.$kfm_db_prefix.'users');
   $image_path_changed = false;
   setcookie( 'kfm_last_opened_dir' );
   echo '<html><head>
  <meta http-equiv="Refresh" content="0; url='.$_server["request_uri"].'">
</head><body>';
echo '<p>Your settings have been applied. Please reload this window.</p>';
echo '</body></html>';
   exit;
}
// }
...
// }
// { JSON
if (!function_exists('kfm_json_encode')) { // php-json is not installed
...
//added zUhrikova 25/02/2010 keeping our banned folders, skipping the database entries
$sBanfol = "";//$kfm_workdirectory;
foreach( $kfmAdd_banned_folders as $fld )
  $sBanfol = $sBanfol . "," . $fld;
  
$settings['banned_folders']  = $sBanfol;
...
/// Addition  2010/06/30
if( isset( $aFckOptions["kfmlang"] ) && $aFckOptions["kfmlang"] != 'en' && $aFckOptions["kfmlang"] != 'auto' ) {
  $kfm_language = $aFckOptions["kfmlang"];
}
/// End of addition
...
function kfmAdd_changeDirectory($id) {
    include_once KFM_BASE_PATH.'includes/directories.php';
    return _AddchangeDirectory($id);
}
...
### Added zUhrikova from the old version by pBaran		3/2/2010		Foliovision
## Function to get image size from url
function kfm_getImageSize( $id ){//$strUrl
      //echo("\n\n".$strUrl."\n\n");
		require_once(KFM_BASE_PATH.'includes/images.php');
		return _getImageSize( $id );//$strUrl
	}
...
function kfm_getFilesUrlAndSizes( $id, $x=0, $y=0 ){
    global $kfm;
		...
		$aReturn[] = getimagesize($_SERVER['DOCUMENT_ROOT'] .str_replace( $kfm->setting('real_files_url'), $kfm->setting('files_url'), $aReturn[0] ) );//$aReturn[0]
  	   $aReturn[] = _getFileUrl( $id, 0, 0 );
      $aReturn[] = _getImageSize( $id );//$aReturn[2]
      return $aReturn;
	}
### End of add		zUhrikova		3/2/2010
// }
...
function kfm_move_uploaded_file( $filename, $to, $tmpname ){
	global $kfm,$kfm_only_allow_image_upload, $kfm_session;
	if( empty( $to ) ){
		unlink( $tmpname );
		return true;
	}
	...
	if( !file_exists( $to ) ) return kfm_lang( 'failedToSaveTmpFile', $tmpname, $to );
		...
		if( function_exists( 'exif_imagetype' ) ){
			...
			if( $imgtype ){
				...
				else if ( $imgtype != 3 ){  //cant retrieve exif from png like this
...
// added zUhrikova 08/03/2010
function kfm_SetFileDetailsCookie( $iState ){
	$bCookie = setcookie( 'kfm_file_details_state', $iState, time() + 60 * 60 * 24 * 90, '/' );
	return $bCookie;
}
function kfm_SetSearchPanelCookie( $iState ){
	$bCookie = setcookie( 'kfm_search_panel_state', $iState, time() + 60 * 60 * 24 * 90, '/' );
	return $bCookie;
}
function kfm_SetDirectoriesPanelCookie( $iState ){
	$bCookie = setcookie( 'kfm_directories_panel_state', $iState, time() + 60 * 60 * 24 * 90, '/' );
	return $bCookie;
}
