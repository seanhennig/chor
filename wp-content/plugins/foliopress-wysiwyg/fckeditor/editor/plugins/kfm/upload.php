	...
	foreach($files as $file){
		...
		if(!$cwd) {
			...
			if (!is_file($sTestFile)) {
				$errors[] = 'No file uploaded.';//Dir->path().'/'.$tmpname
			}
		...
		if (!$replace && file_exists($to)){
			...
			if( is_uploaded_file( $tmpname ) ) {
				...
				$strRenameTemp = basename( $tmpname );
			}
		...
		if (!count($errors)) {
			if ($bRenaming) {
				kfm_move_uploaded_file( $filename, $to, $toDir->path().$tmpname );
			} else {
				kfm_move_uploaded_file( $filename, $to, $tmpname );
			}
			
			//move_uploaded_file($tmpname, $to);//$toDir->path().
			if (!file_exists($to)) {
				$errors[] = kfm_lang('failedToSaveTmpFile' , $toDir->path().$tmpname, $to);//file_exists($to)
			}
			else {
					...
					if (function_exists('exif_imagetype')) {
						...
						if ($imgtype) {
							...
							if ($imgtype==1) { // gif
								...
								if (is_array($data)&&isset($data['COMMENT'])&&is_array($data['COMMENT'])) {
									$comment = join("\n", $data['COMMENT']);
								}
							}
