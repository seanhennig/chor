   ...
   */
   ...
   function getContent(){
	...
	function getUrl($x=0,$y=0){
		global $kfm, $kfm_userfiles_output, $kfm_userfiles_output_real, $kfm_workdirectory, $kfm_special_thumbs_sizes;
    ...
    global $kfm_userfiles_output;
		...
		/// Addition  2011/01/17
    if( $kfm->setting('real_files_url') ) {
      $path = $kfm->setting('real_files_url');
    } else {
      $path = $kfm->setting('files_url');
    }
    /// End of addition
		...
		if($this->isImage()&&$x&&$y){ // thumbnail is requested
			...
			$strX = sprintf( "%d", $x );
               ...
               //$strPath = $kfm_userfiles_output . str_replace( $rootdir, '', $this->directory ) . $x . '/' . $this->name;
					$strPath =  $path . $this->dir->relativePath().'/'. $x . '/' . $this->name;//
					$strPath = str_replace('//','/',$strPath);
					return $strPath;
				}else{
					$img->setThumbnail($x,$y);
					if( strlen( $kfm_userfiles_output_real ) > 0 ) {
					 return $kfm_userfiles_output_real.$kfm_workdirectory.'/thumbs/'.$img->thumb_id;
					} else {
